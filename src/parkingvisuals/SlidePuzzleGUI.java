...
class SlidePuzzleGUI extends JPanel {
	...
	class GraphicsPanel extends JPanel implements MouseListener {
		...
		public void paintComponent(Graphics g) {
			...
			String open = ("Number of spaces occupied: " + occupiedCount());
			infoDump.setText(open);
			...
			// toggles whether or not clicking a given platform gives you
			// information
		...
		public void mousePressed(MouseEvent e) {
			...
			if (_puzzleModel.getSpotTaken(row, col) == true) {
				...
				if (col == 4 && row == 4) {
					...
					// } else if (col == 0) {
					// for (int i =
					// _puzzleModel._emptyTile.getCol();i>=col;i--){
					// _puzzleModel.moveTileWest();
					// _puzzleGraphics.repaint();
					// }
					// for (int i = _puzzleModel._emptyTile.getRow();i>row;i--){
		...
		public void actionPerformed(ActionEvent e) {
			if (stillRunning == true) {
				...
				String infoDumpTextShutdown = (infoDumpText + "\nAVCPP is currently shut down. Press RESUME"
						+ " OPERATIONS to resume normal operations." + "\n\nNumber of spaces occupied: " + occupiedCount() + "\nSpaces Occupied: "
						+ occupiedSpots());
				infoDump.setText(infoDumpTextShutdown);
	...
	public int occupiedCount() {
		...
		LinkedList occupiedCarSpots = new LinkedList<>();
		int count = 0;
		for (int r = 0; r < 6 - 1; r++) {
			for (int c = 0; c < 5; c++) {
				if (_puzzleModel.getSpotTaken(r, c) == true) {
					...
					count++;
					occupiedCarSpots.add(count);
				}
			}
		}

		return occupiedCarSpots.size();
	}	
	
	public LinkedList occupiedSpots() {
		...
		for (int r = 0; r < 6 - 1; r++) {
			for (int c = 0; c < 5; c++) {
				if (_puzzleModel.getSpotTaken(r, c) == true) {
					...
					//count++;
					occupiedCarSpots.add(_puzzleModel.getStorageNumber(r, c));
				}
			}
		}
